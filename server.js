...
function startNight(salon) {
  ...
  salon.log.push({
    type: "system",
    text: `🌙 Nuit ${game.round} — Le village s'endort…`,
  });
...
function resolveNight(salon) {
    ...
    if (cupidon) {
      ...
      if (a && a.startsWith("cupidon:")) {
        ...
        if (ids.length === 2) {
          ...
          game.lovers = ids;
          results.push(
            `💘 Cupidon a uni ${names.join(" et ")} pour l'éternité.`,
          );
        }
      }
    }
  ...
  if (salvateur) {
    ...
    if (a && a.startsWith("protect:")) {
      ...
      if (t) {
        t.protected = true;
        results.push(`🛡 Le Salvateur veille cette nuit…`);
      }
    }
  }
  ...
  const infect = players.find(
    (p) => p.role === "INFECT" && p.alive && !p.infectUsed,
  );
  if (
    infect &&
    game.nightActions[infect.id] &&
    game.nightActions[infect.id].startsWith("infect:")
  ) {
    ...
    if (t && !t.protected) {
      ...
      results.push(
        `🩸 Le Maudit a transmis sa malédiction… et périt à l'aube.`,
      );
    ...
    if (vp?.alive && !vp.protected) {
      ...
      if (grandLoup) {
        ...
        if (a2) {
          const v2 = players.find(
            (p) => p.id === a2 && p.alive && !p.protected && p.id !== victimId,
          );
          if (v2) {
            v2.alive = false;
            deaths.push(v2.id);
            results.push(
              `🐾 L'Alpha frappe une seconde fois : ${v2.name} est mort.`,
            );
          }
        }
      }
        ...
        game.lovers
          ...
          .forEach((lid) => {
            ...
            if (lover) {
              ...
              results.push(`💔 ${lover.name}, fou de chagrin, meurt d'amour.`);
            }
          });
      }
  ...
  if (sorciere) {
    ...
    if (a && a.startsWith("death:")) {
      ...
      if (t) {
        t.alive = false;
        deaths.push(t.id);
        game.witchPotions = { ...game.witchPotions, death: false };
        results.push(`🧪 La Sorcière a empoisonné ${t.name}.`);
      }
    } else if (a && a.startsWith("life:")) {
      const tid = a.split(":")[1];
      const t = players.find((p) => p.id === tid);
      if (t && !t.alive && deaths.includes(tid)) {
        t.alive = true;
        deaths.splice(deaths.indexOf(tid), 1);
        game.witchPotions = { ...game.witchPotions, life: false };
        results.push(`💚 La Sorcière a sauvé ${t.name} in extremis !`);
      }
    }
  }
  ...
  if (corbeau) {
    ...
    if (a && a.startsWith("corbeau:")) {
      const t = players.find((p) => p.id === a.split(":")[1]);
      if (t) {
        game.corbeauTarget = t.id;
        results.push(
          `🐦‍⬛ Le Corbeau a désigné ${t.name} — +2 votes contre lui aujourd'hui.`,
        );
      }
    }
  } else {
...
function startDay(salon) {
  ...
  salon.log.push({
    type: "system",
    text: `☀️ Jour ${game.round} — Le village se réunit. Débattez et votez !`,
  });
...
function resolveDay(salon) {
  ...
  if (elimId) {
    ...
    if (elim?.alive) {
      if (elim.role === "IDIOT") {
        results.push(
          `🃏 Le village a voté contre ${elim.name} — c'est le Fou du Roi ! Il survit mais perd son droit de vote.`,
        );
        elim.idiotNoVote = true;
      } else {
        elim.alive = false;
        results.push(
          `⚖️ Le village a éliminé ${elim.name}. C'était : ${getRoleNameServer(elim.role)}.`,
        );
        // Chasseur
        if (elim.role === "CHASSEUR")
          results.push(
            `🏹 Le Chasseur tire une dernière flèche ! (désigné par l'hôte)`,
          );
        // Ange
        if (elim.role === "ANGE" && game.round <= 2) {
          endGame(salon, "ANGE");
          return;
        }
        // Amoureux
        if (elim.lover && game.lovers) {
          game.lovers
            .filter((id) => id !== elim.id)
            .forEach((lid) => {
              const lover = players.find((p) => p.id === lid && p.alive);
              if (lover) {
                lover.alive = false;
                results.push(
                  `💔 ${lover.name}, fou de chagrin, meurt d'amour.`,
                );
              }
            });
        }
  ...
  results.forEach((r) =>
    salon.log.push({
      type: r.includes("éliminé") || r.includes("meurt") ? "death" : "info",
      text: r,
    }),
  );
...
function endGame(salon, winner) {
  ...
  salon.log.push({
    type: "system",
    text: `🏆 FIN DE PARTIE — ${msgs[winner] || winner}`,
  });
...
function checkWin(players, game) {
  ...
  // Ménestrel
  if (game.charmed && alive.every((p) => game.charmed.includes(p.id)))
    return "FLUTE";
  // Loup Blanc seul
...
app.post("/api/salon/start", (req, res) => {
  ...
  salon.log.push({
    type: "system",
    text: "🎭 La partie commence ! Regardez votre rôle…",
  });
  // Timer reveal
...
app.post("/api/salon/vote", (req, res) => {
  ...
  if (voter.idiotNoVote)
    return res.json({
      ok: false,
      error: "Vous ne pouvez plus voter (Fou du Roi)",
    });
...
app.post("/api/salon/chat", (req, res) => {
  ...
  if (salon.game?.phase === "NIGHT" && player.alive)
...
function publicSalon(salon, playerId) {
  ...
  const players = salon.players.map((p) => {
    ...
    return {
      ...
      ...(salon.game?.phase === "NIGHT" &&
      p.role &&
      ["LOUP", "GRAND_LOUP", "LOUP_VOYANT", "INFECT"].includes(p.role) &&
      playerId &&
      ["LOUP", "GRAND_LOUP", "LOUP_VOYANT", "INFECT"].includes(
        salon.players.find((x) => x.id === playerId)?.role,
      )
        ? { role: p.role }
        : {}),
    };
